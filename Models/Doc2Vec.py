...
from TextPreProcessing import nltk_tokenizer as nl_token
...
def get_doc2vec_similarity(req_doc,list_of_resume):
    ...
    inferredVector = model.infer_vector(test_corpus[0])
    simalrityScores = model.docvecs.most_similar([inferredVector], topn=len(model.docvecs))
    return simalrityScores
